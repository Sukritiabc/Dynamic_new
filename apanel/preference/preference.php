        ...
        <div class="form-row">
            ...
            <!--logo image 2-->
             <div class="form-row hide">
                ...
                <?php if (!empty($PrefeRow->logo_upload2)): ?>
                    <div class="col-md-2" id="removeSavedimg1000">
                        <div class="infobox info-bg">
                            <div class="button-group" data-toggle="buttons">
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/preference/" . $PrefeRow->logo_upload2)):
                                    $filesize = filesize(SITE_ROOT . "images/preference/" . $PrefeRow->logo_upload2);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                <a class="btn small float-right" href="javascript:void(0);"
                                   onclick="deleteSavedPreferenceimage(1000);">
                                    <i class="glyph-icon icon-trash-o"></i>
                                </a>
                            </div>
                            <img src="<?php echo IMAGE_PATH . 'preference/thumbnails/' . $PrefeRow->logo_upload2; ?>"
                            
                                 style="width:100%"/>
                                 <input type="hidden" name="imageArraynamelogo" value="" class=""/>
                        </div>
                    </div>
                <?php endif; ?>
                <div class="form-input col-md-10 uploader1000 <?php echo !empty($PrefeRow->logo_upload2) ? "hide" : ""; ?>">
                    <input type="file" name="logo_upload2" id="logo_upload2" class="transparent no-shadow">
                    <label>
                        <small>Image Dimensions (<?php echo Module::get_properties($moduleId, 'simgwidth'); ?> px
                            X <?php echo Module::get_properties($moduleId, 'simgheight'); ?> px)
                        </small>
        ...
        $('#logo_upload2').uploadify({
            ...
            'onUploadSuccess': function (file, data, response) {
                ...
                $.post('<?php echo BASE_URL;?>apanel/preference/uploaded_image3.php', {imagefile: filename}, function (msg) {
                    $('#preview_Imagelogo').html(msg).show();
                });
