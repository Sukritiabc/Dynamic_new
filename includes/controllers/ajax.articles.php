...
//			$record->linktype 	= $_REQUEST['linktype'];
		...
		case "delete":
			...
			echo json_encode([
				"action" => "error",
				"message" => "Article About Us cannot be deleted."
			]);
			break;
		}
			$db->begin();
		...
		case "bulkDelete":
			...
			for ($i = 1; $i < count($allid); $i++) {
				...
				$articleId = (int) $allid[$i];

				// Skip protected article
				if ($articleId === 0 || $articleId === 21) {
					$errors[] = "Article About Us cannot be deleted.";
					continue;
				}

				$res = $db->query("DELETE FROM tbl_articles WHERE id = '{$articleId}'");

				if ($res) {
					deleteSlug('Article', $articleId);
					$deleted = true;
				}
			}
			...
			if ($deleted && !empty($errors)) {
				echo json_encode([
					"action"  => "warning",
					"message" => "Some records deleted, but some were skipped.<br>" . implode("<br>", $errors)
				]);
			} elseif ($deleted) {
				echo json_encode([
					"action"  => "success",
				...
				echo json_encode([
					"action"  => "error",
					"message" => $GLOBALS['basic']['noRecords']
				]);
			}
